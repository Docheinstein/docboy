...
DebuggableGameBoy::Builder::Builder() :
    bootRom(),
    frequency() {
}
...
DebuggableGameBoy::DebuggableGameBoy(std::unique_ptr<IBootROM> bootRom_, uint64_t frequency) :
    ...
    bus(vram, wram1, wram2, oam, hram, cartridgeSlot, boot, joypad, serialPort, timers, interrupts, sound,
        lcdController, boot),
