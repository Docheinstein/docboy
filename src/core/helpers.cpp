...
constexpr const char* IO_MNEMONICS[256] = {
    "P1",      "SB",          "SC",           "UNKNOWN",     "DIV",          "TIMA",     "TMA",     "TAC",
    "UNKNOWN", "UNKNOWN",     "UNKNOWN",      "UNKNOWN",     "UNKNOWN",      "UNKNOWN",  "UNKNOWN", "IF",
    "NR10",    "NR11",        "NR12",         "NR13",        "NR14",         "UNKNOWN",  "NR21",    "NR22",
    "NR23",    "NR24",        "NR30",         "NR31",        "NR32",         "NR33",     "NR34",    "UNKNOWN",
    "NR41",    "NR42",        "NR43",         "NR44",        "NR50",         "NR51",     "NR52",    "UNKNOWN",
    "UNKNOWN", "UNKNOWN",     "UNKNOWN",      "UNKNOWN",     "UNKNOWN",      "UNKNOWN",  "UNKNOWN", "UNKNOWN",
    "WAVE 0",  "WAVE 1",      "WAVE 2",       "WAVE 3",      "WAVE 4",       "WAVE 5",   "WAVE 6",  "WAVE 7",
    "WAVE 8",  "WAVE 9",      "WAVE A",       "WAVE B",      "WAVE C",       "WAVE D",   "WAVE E",  "WAVE F",
    "LCDC",    "STAT",        "SCY",          "SCX",         "LY",           "LYC",      "DMA",     "BGP",
    "OBP0",    "OBP1",        "WY",           "WX",          "UNKNOWN",      "KEY1",     "UNKNOWN", "VBK",
    "BOOT",    "HDMA SRC HI", "HDMA SRC LOW", "HDMA DST HI", "HDMA DST LOW", "HDMA LEN", "RP",      "UNKNOWN",
    "UNKNOWN", "UNKNOWN",     "UNKNOWN",      "UNKNOWN",     "UNKNOWN",      "UNKNOWN",  "UNKNOWN", "UNKNOWN",
    "UNKNOWN", "UNKNOWN",     "UNKNOWN",      "UNKNOWN",     "UNKNOWN",      "UNKNOWN",  "UNKNOWN", "UNKNOWN",
    "BCPS",    "BCPD",        "OCPS",         "OCPD",        "UNKNOWN",      "UNKNOWN",  "UNKNOWN", "UNKNOWN",
    "SVBK",    "UNKNOWN",     "UNKNOWN",      "UNKNOWN",     "UNKNOWN",      "UNKNOWN",  "PCM12",   "PCM34",
    "UNKNOWN", "UNKNOWN",     "UNKNOWN",      "UNKNOWN",     "UNKNOWN",      "UNKNOWN",  "UNKNOWN", "UNKNOWN",
};

uint8_t instruction_length(uint8_t opcode, bool cb) {
    return cb ? INSTRUCTIONS_CB[opcode].length : INSTRUCTIONS[opcode].length;
}
...
uint8_t instruction_length(const std::vector<uint8_t>& instruction) {
    ...
    if (instruction[0] == 0xCB) {
        if (instruction.size() < 2)
            return 0;
        return instruction_length(instruction[1], true);
    }
    return instruction_length(instruction[0]);
}

std::pair<uint8_t, uint8_t> instruction_duration(uint8_t opcode, bool cb) {
    if (cb) {
        auto [min, max] = INSTRUCTIONS_CB[opcode].duration;
        return std::make_pair(min, max);
    }
    auto [min, max] = INSTRUCTIONS[opcode].duration;
    return std::make_pair(min, max);
}
...
std::pair<uint8_t, uint8_t> instruction_duration(const std::vector<uint8_t>& instruction) {
    ...
    if (instruction[0] == 0xCB) {
        if (instruction.size() < 2)
            return {0, 0};
        return instruction_duration(instruction[1], true);
    }
    return instruction_duration(instruction[0]);
}

std::string instruction_mnemonic(uint8_t opcode, bool cb) {
    // not formatted
    if (cb)
        return INSTRUCTIONS_CB[opcode].mnemonic;
...
std::string instruction_mnemonic(const std::vector<uint8_t>& instruction, uint16_t address) {
    ...
    case 0xF0: {
        ...
        char buf[32];
        snprintf(buf, sizeof(buf), INSTRUCTIONS[opcode].mnemonic, instruction[1],
                 address_mnemonic(0xFF00 + instruction[1]).c_str());
        return buf;
    }
    default:
        return INSTRUCTIONS[opcode].mnemonic;
...
std::string address_mnemonic(uint16_t address) {
    // TODO: mnemonic for other known stuff
    if (address >= MemoryMap::IO::START && address <= MemoryMap::IO::END)
        return IO_MNEMONICS[address - MemoryMap::IO::START];
    else if (address >= MemoryMap::HRAM::START && address <= MemoryMap::HRAM::END)
        return "HRAM";
