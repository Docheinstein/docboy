...
bool CartridgeHeader::isValid() const {
...
CartridgeHeader CartridgeHeader::parseHeader(const IMemory& memory) {
    ...
    for (uint16_t addr = 0x100; addr <= 0x14F; addr++) {
        headerData.push_back(memory.read(addr));
    }

    auto raw_data = (uint8_t*)headerData.data();

    CartridgeHeader h;

    // Raw data
    memcpy_range_v(h.data, raw_data, 0x000, 0x04F);

    // Entry point (0x100 - 0x103)
    memcpy_range(h.entry_point.data(), raw_data, 0x000, 0x003);

    // Nintendo Logo (0x104 - 0x133)
    memcpy_range(h.nintendo_logo.data(), raw_data, 0x004, 0x033);

    // Title (0x134 - 0x143)
    memcpy_range(h.title.data(), raw_data, 0x034, 0x043);

    // CGB flag (0x143)
    h.cgb_flag = raw_data[0x043];

    // New licensee code (0x144 - 0x145)
    memcpy_range(h.new_licensee_code.data(), raw_data, 0x044, 0x045);

    // SGB flag (0x146)
    h.sgb_flag = raw_data[0x046];

    // Cartridge type (0x147)
    h.cartridge_type = raw_data[0x047];

    // Cartridge type (0x148)
    h.rom_size = raw_data[0x048];

    // Cartridge type (0x149)
    h.ram_size = raw_data[0x049];

    // Destination code (0x14A)
    h.destination_code = raw_data[0x04A];

    // Old licensee code (0x14B)
    h.old_licensee_code = raw_data[0x04B];

    // ROM version number (0x14C)
    h.rom_version_number = raw_data[0x04C];

    // Header checksum (0x14D)
    h.header_checksum = raw_data[0x04D];

    // Global checksum (0x14E - 0x14F)
    memcpy_range(h.global_checksum.data(), raw_data, 0x04E, 0x04F);
