...
Bus::Bus(IMemory &vram,
        ...
        ILCDIO &lcd,
        IBootIO &boot) :
     vram(vram), wram1(wram1), wram2(wram2), oam(oam), hram(hram),
     cartridge(cartridge), bootRom(bootRom),
     joypad(joypad), serial(serial), timers(timers), interrupts(interrupts),
     sound(sound), lcd(lcd), boot(boot), io {
