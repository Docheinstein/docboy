...
Mmu::Mmu(IF_BOOTROM(BootRom& bootRom COMMA) ExtBus& extBus, CpuBus& cpuBus, VramBus& vramBus, OamBus& oamBus) :
    ...
    for (uint16_t i = Specs::MemoryLayout::BOOTROM::START; i <= Specs::MemoryLayout::BOOTROM::END; i++) {
        initAccessors(i, &cpuBus);
    }
#else
    /* 0x0000 - 0x00FF */
    for (uint16_t i = Specs::MemoryLayout::BOOTROM::START; i <= Specs::MemoryLayout::BOOTROM::END; i++) {
        initAccessors(i, &extBus);
    }
#endif
    ...
    /* 0xFFFF */
    initAccessors(Specs::MemoryLayout::IE, &cpuBus);
}
...
Mmu::BusAccess Mmu::makeAccessors(Bus* bus) {
    ...
    return BusAccess {bus,
                      {BusAccess::readRequest<Bus, Dev>, BusAccess::flushReadRequest<Bus, Dev>,
...
void Mmu::initAccessors(uint16_t address, Bus* bus) {
    busAccessors[Device::Cpu][address] = makeAccessors<Device::Cpu>(bus);
    busAccessors[Device::Dma][address] = makeAccessors<Device::Dma>(bus);
}
...
void Mmu::saveState(Parcel& parcel) const {
    ...
    IF_BOOTROM(parcel.writeBool(bootRomLocked));
}
