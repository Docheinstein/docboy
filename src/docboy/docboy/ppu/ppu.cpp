...
constexpr uint8_t CGB_PALETTE_SIZE = 8;
constexpr uint8_t CGB_NUM_DMG_MODE_PALETTES = 8;

struct DmgModePalettes {
    uint8_t bgp0[CGB_PALETTE_SIZE] {};
    uint8_t objp0[CGB_PALETTE_SIZE] {};
    uint8_t objp1[CGB_PALETTE_SIZE] {};
};
...
constexpr uint8_t DMG_MODE_AMBIGUOUS_TITLE_CHECKSUMS_START_INDEX = 0x41;

constexpr uint8_t DMG_MODE_TITLE_CHECKSUMS[] {/* 0x00 */ 0x00, 0x88, 0x16, 0x36, 0xD1, 0xDB, 0xF2, 0x3C,
...
constexpr std::optional<uint8_t> DMG_MODE_TITLE_FOURTH_LETTER_DISAMBIGUATION[][3] {
    /* 0x41 */ {'B', 'U', 'R'},          /* 0xB3 */
    /* 0x42 */ {'E', 'R', std::nullopt}, /* 0x46 */
    /* 0x43 */ {'F', 'A', std::nullopt}, /* 0x28 */
    /* 0x44 */ {'A', 'R', std::nullopt}, /* 0xA5 */
    /* 0x45 */ {'A', ' ', std::nullopt}, /* 0xC6 */
    /* 0x46 */ {'R', 'I', std::nullopt}, /* 0xD3 */
    /* 0x47 */ {'B', 'N', std::nullopt}, /* 0x27 */
    /* 0x48 */ {'E', 'A', std::nullopt}, /* 0x61 */
    /* 0x49 */ {'K', 'I', std::nullopt}, /* 0x18 */
    /* 0x4A */ {'E', 'L', std::nullopt}, /* 0x66 */
    /* 0x4B */ {'K', 'I', std::nullopt}, /* 0x6A */
    /* 0x4C */ {' ', 'C', std::nullopt}, /* 0xBF */
    /* 0x4D */ {'R', 'E', std::nullopt}, /* 0x0D */
    /* 0x4E */ {'-', ' ', std::nullopt}, /* 0xF4 */
};

constexpr uint8_t DMG_MODE_PALETTE_TRIPLET_IDS_AND_FLAGS[] {/* 0x00 */ 0x7C, 0x08, 0x12, 0xA3, 0xA2, 0x07, 0x87, 0x4B,
...
constexpr uint8_t DMG_MODE_PALETTE_OFFSETS[] {
    /* 0x00 */ 0x80, 0xB0, 0x40,
    /* 0x01 */ 0x88, 0x20, 0x68,
    /* 0x02 */ 0xDE, 0x00, 0x70,
    /* 0x03 */ 0xDE, 0x20, 0x78,
...
constexpr uint8_t DMG_MODE_PALETTES[] {
...
constexpr DmgModePalettes compute_dmg_mode_non_ambiguous_palettes(uint8_t cksum_index) {
    ...
    uint8_t palette_index = keep_bits<5>(DMG_MODE_PALETTE_TRIPLET_IDS_AND_FLAGS[cksum_index]);
    uint8_t shuffling_flags = get_bits_range<7, 5>(DMG_MODE_PALETTE_TRIPLET_IDS_AND_FLAGS[cksum_index]);

    // - Use the palette index to retrieve the offsets of the real palettes to use
    uint8_t palette_offset_0 = DMG_MODE_PALETTE_OFFSETS[3 * palette_index + 0];
    uint8_t palette_offset_1 = DMG_MODE_PALETTE_OFFSETS[3 * palette_index + 1];
    uint8_t palette_offset_2 = DMG_MODE_PALETTE_OFFSETS[3 * palette_index + 2];

    uint8_t palette_0[CGB_PALETTE_SIZE] {};
    uint8_t palette_1[CGB_PALETTE_SIZE] {};
    uint8_t palette_2[CGB_PALETTE_SIZE] {};

    // - Copy the palettes based on the offsets
    memcpy_uint8(palette_0, &DMG_MODE_PALETTES[palette_offset_0], CGB_PALETTE_SIZE);
    memcpy_uint8(palette_1, &DMG_MODE_PALETTES[palette_offset_1], CGB_PALETTE_SIZE);
    memcpy_uint8(palette_2, &DMG_MODE_PALETTES[palette_offset_2], CGB_PALETTE_SIZE);

    // - Use the shuffling flags to assign OBJP0 and OBJP1
    if (shuffling_flags == 0b000) {
        memcpy_uint8(palette.objp0, palette_2, CGB_PALETTE_SIZE);
        memcpy_uint8(palette.objp1, palette_2, CGB_PALETTE_SIZE);
    } else if (shuffling_flags == 0b001) {
        memcpy_uint8(palette.objp0, palette_0, CGB_PALETTE_SIZE);
        memcpy_uint8(palette.objp1, palette_2, CGB_PALETTE_SIZE);
    } else if (shuffling_flags == 0b010) {
        memcpy_uint8(palette.objp0, palette_2, CGB_PALETTE_SIZE);
        memcpy_uint8(palette.objp1, palette_0, CGB_PALETTE_SIZE);
    } else if (shuffling_flags == 0b011) {
        memcpy_uint8(palette.objp0, palette_0, CGB_PALETTE_SIZE);
        memcpy_uint8(palette.objp1, palette_0, CGB_PALETTE_SIZE);
    } else if (shuffling_flags == 0b0100) {
        memcpy_uint8(palette.objp0, palette_2, CGB_PALETTE_SIZE);
        memcpy_uint8(palette.objp1, palette_1, CGB_PALETTE_SIZE);
    ...
    memcpy_uint8(palette.bgp0, palette_2, CGB_PALETTE_SIZE);

    return palette;
}

constexpr DmgModePalettes DMG_MODE_DEFAULT_PALETTES = compute_dmg_mode_non_ambiguous_palettes(0);
...
constexpr std::array<DmgModePalettesByChecksum, 256> compute_dmg_mode_palettes() {
    ...
    for (uint16_t i = 0; i < 256; i++) {
        ...
        while (cksum_index < array_size(DMG_MODE_TITLE_CHECKSUMS) && DMG_MODE_TITLE_CHECKSUMS[cksum_index] != cksum) {
            cksum_index++;
        }

        if (cksum_index == array_size(DMG_MODE_TITLE_CHECKSUMS)) {
            // Checksum not found: use index 0 by default
            cksum_index = 0;
        }

        if (cksum_index < DMG_MODE_AMBIGUOUS_TITLE_CHECKSUMS_START_INDEX) {
            ...
            const auto& disambiguation_letters =
                DMG_MODE_TITLE_FOURTH_LETTER_DISAMBIGUATION[cksum_index -
                                                            DMG_MODE_AMBIGUOUS_TITLE_CHECKSUMS_START_INDEX];
            ...
            for (uint8_t letter_index = 0; letter_index < 3; letter_index++) {
                if (std::optional<uint8_t> optional_letter = disambiguation_letters[letter_index]) {
                    uint8_t non_ambiguous_cksum_index =
                        cksum_index + letter_index * array_size(DMG_MODE_TITLE_FOURTH_LETTER_DISAMBIGUATION);
...
DmgModePalettes compute_dmg_mode_palettes_from_header(const CartridgeHeader& header) {
    static constexpr std::array<DmgModePalettesByChecksum, 256> DMG_MODE_DMG_MODE_PALETTES_BY_CHECKSUM =
        compute_dmg_mode_palettes();

    uint8_t cksum = CartridgeHeaderHelpers::title_checksum(header);
    const auto& palettes_of_checksum = DMG_MODE_DMG_MODE_PALETTES_BY_CHECKSUM[cksum];
    ...
    return DMG_MODE_DEFAULT_PALETTES;
}
...
void load_dmg_mode_palettes_from_header(const CartridgeHeader& header, uint8_t* bg_palettes, uint8_t* obj_palettes) {
    ...
    memcpy(bg_palettes, palettes.bgp0, CGB_PALETTE_SIZE);
    memcpy(obj_palettes, palettes.objp0, CGB_PALETTE_SIZE);
    memcpy(obj_palettes + CGB_PALETTE_SIZE, palettes.objp1, CGB_PALETTE_SIZE);

    // Other BG palettes are all initialized as white (0x7FFF)
    for (uint32_t i = CGB_PALETTE_SIZE; i < CGB_NUM_DMG_MODE_PALETTES * CGB_PALETTE_SIZE; i += 2) {
        bg_palettes[i] = 0xFF;
        bg_palettes[i + 1] = 0x7F;
    }

    // Other OBJ palettes are all initialized as black (0x0000)
    memset(obj_palettes + 2 * CGB_PALETTE_SIZE, 0, (CGB_NUM_DMG_MODE_PALETTES - 2) * CGB_PALETTE_SIZE);
}
#endif
