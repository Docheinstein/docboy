...
ARGUMENTUM_INLINE ArgumentHelpResult ArgumentDescriber::describe_argument(const ParserDefinition& parserDef,
...
ARGUMENTUM_INLINE std::vector<ArgumentHelpResult>
ArgumentDescriber::describe_arguments(const ParserDefinition& parserDef) const {
...
ARGUMENTUM_INLINE ArgumentHelpResult ArgumentDescriber::describeCommand(const Command& command) const {
    ArgumentHelpResult help;
    help.isCommand = true;
...
ARGUMENTUM_INLINE std::string ArgumentDescriber::describeArguments(const Option& option,
    ...
    if (mmax < mmin) {
        ...
        if (remaining == 1)
            ...
            res << getOpenBracket(res) << getMetavar(ivar) << " {0.." << std::to_string(remaining) << "}";
        }
    }
