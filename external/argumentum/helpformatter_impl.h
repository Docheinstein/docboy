...
ARGUMENTUM_INLINE size_t HelpFormatter::deriveMaxArgumentWidth(const std::vector<ArgumentHelpResult>& args) const {
    if (args.empty())
        return 0U;

    auto imax = std::max_element(std::begin(args), std::end(args), [this](auto&& a, auto&& b) {
...
ARGUMENTUM_INLINE void HelpFormatter::formatUsage(const ParserDefinition& parserDef,
                                                  std::vector<ArgumentHelpResult>& args, Writer& writer) {
