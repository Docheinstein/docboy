...
ARGUMENTUM_INLINE std::string Option::getHelpName() const {
    if (isPositional()) {
        ...
        const auto& name = !mMetavar.empty() ? mMetavar[0] : (!mLongName.empty() ? mLongName : mShortName);
...
ARGUMENTUM_INLINE std::vector<std::string> Option::getMetavar() const {
    ...
    std::transform(metavar.begin(), metavar.end(), metavar.begin(), isPositional ? tolower : toupper);
    return {metavar};
}
